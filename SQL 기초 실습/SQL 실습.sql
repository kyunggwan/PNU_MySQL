...
SELECT userID, SUM(amount * price) AS '총 구매 액' 
FROM buytbl 
GROUP BY userID;
...
SELECT DISTINCT u.name, u.addr, u.mobile1, u.mobile2 
FROM usertbl u 
JOIN buytbl b 
...
FROM usertbl 
WHERE userID = (SELECT DISTINCT u.userID FROM usertbl u 
JOIN buytbl b 
ON u.userID = b.userID AND u.userID = 'BBK'); 